...
static NSString *LNKeychainUtilsErrorDomain = @"LNKeychainUtilsErrorDomain";
...
@implementation LNKeychainUtils
...
+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
	if (!username || !serviceName) {
		if (error != nil) {
			*error = [NSError errorWithDomain: LNKeychainUtilsErrorDomain code: -2000 userInfo: nil];
		}
		return nil;
	}
	...
	if (status != noErr) {
		...
		if (error != nil && status != errSecItemNotFound) {
			...
			*error = [NSError errorWithDomain: LNKeychainUtilsErrorDomain code: status userInfo: nil];
		}
	...
	if (status != noErr) {
		if (status == errSecItemNotFound) {
			...
			if (error != nil) {
				*error = [NSError errorWithDomain: LNKeychainUtilsErrorDomain code: -1999 userInfo: nil];
			}
		}
		else {
...
{		
	...
	NSString *existingPassword = [LNKeychainUtils getPasswordForUsername: username andServiceName: serviceName error:&getError];
